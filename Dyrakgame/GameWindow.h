...
namespace Dyrakgame {
    ...
    private:
        void CheckGameEnd() {
            ...
            // Поражение (бот пуст, у игрока ещё есть карты)
            else if (botCount == 0 && playerCount > 0) {
                msg = "Вы проиграли, вы ДУРАК!!!";
            }
            // Ничья (никто не остался без карт одновременно)
            else if (playerCount == 0 && botCount == 0) {
                msg = "Ничья, никто не дурак!!!";
            }
        ...
        void UpdateDeckCountLabel() {
            ...
            deckCountLabel->Text = String::Format("В колоде: {0}", remaining);
        }
